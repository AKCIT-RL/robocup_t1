...
## Simulation on AMD/x86_64 (Docker)

If you are running the project on a desktop PC (e.g., AMD/Intel CPU with NVIDIA GPU) for simulation, use the standard `dockerfile` located in the root directory.

### 1. Build the x86_64 Image

```bash
docker build -t robocup_demo:2.0 -f dockerfile .
```

### 2. Manual Vision Model Configuration

TensorRT engines are specific to the GPU architecture and won't work across different hardware types (e.g., Orin vs RTX 40). **You must manually update the configuration** to point to the correct model for your system.

**Available Models in `src/vision/model/`:**
*   `best_30.engine`: Optimized for NVIDIA RTX 30 series GPUs.
*   `best_40.engine`: Optimized for NVIDIA RTX 40 series GPUs.
*   `best_orin.engine`: For Jetson Orin (JetPack 6.0).
*   `best_orin_10.3.engine`: For Jetson Orin (JetPack 6.2).

**How to Swap:**
Edit `src/vision/config/config_local.yaml`:
```yaml
detection_model:
  model_path: "./src/vision/model/best_40.engine" # Example for RTX 40 Series
```

### 3. Run Simulation

After everything is set up and compiled:
```bash
