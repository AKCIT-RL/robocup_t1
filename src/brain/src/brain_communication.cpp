...
void BrainCommunication::unicastCommunication() {
    ...
    while (_unicast_communication_flag) {
        ...
        msg.leadSwitchTimer = brain->data->leadSwitchTimer;
        log(format("ImAlive: %d, ImLead: %d, myCost: %.1f, myCmdId: %d, myCmd: %d, LeadSwitch: %.3f", msg.isAlive, msg.isLead, msg.cost, msg.cmdId, msg.cmd, brain->msecsSince(msg.leadSwitchTimer)));
...
void BrainCommunication::spinCommunicationReceiver() {
    ...
    while (_receive_communication_flag) {
        ...
        if (msg.cmdId > brain->data->tmCmdId) {
            ...
            brain->data->leadSwitchTimer = msg.leadSwitchTimer;
