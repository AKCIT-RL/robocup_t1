...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("striker.kick_radius_to_goal", 3.5);
    declare_parameter<double>("striker.avoid_angle_degrees", 25.0);
    declare_parameter<double>("striker.drible_radius", 1.5);
    declare_parameter<double>("striker.kickao_radius", 2.0);
    declare_parameter<double>("striker.drible_circle_degrees", 180.0);
    declare_parameter<bool>("striker.enable_kickao", false);
    declare_parameter<bool>("striker.enable_kick", false);

    declare_parameter<double>("striker.adjust_dists.kickinho", 0.5);
    declare_parameter<double>("striker.adjust_dists.kick", 0.2);
    declare_parameter<double>("striker.adjust_dists.kickao", 1.45);
    declare_parameter<double>("striker.adjust_dists.conduzir", 0.5);
    declare_parameter<double>("striker.adjust_dists.drible", 0.1);
    declare_parameter<double>("striker.adjust_dists.tolerance", 0.2);
...
void Brain::loadConfig()
{
    ...
    get_parameter("strategy.lead_switch_threshold", config->leadSwitchThreshold);
    ...
    get_parameter("striker.kickao_radius", config->kickaoRadius);
    get_parameter("striker.drible_circle_degrees", config->dribleCircleDegrees);
    get_parameter("striker.enable_kickao", config->enableKickao);
    get_parameter("striker.enable_kick", config->enableKick);

    get_parameter("striker.adjust_dists.kickinho", config->adjustDists.kickinho);
    get_parameter("striker.adjust_dists.kick", config->adjustDists.kick);
    get_parameter("striker.adjust_dists.kickao", config->adjustDists.kickao);
    get_parameter("striker.adjust_dists.conduzir", config->adjustDists.conduzir);
    get_parameter("striker.adjust_dists.drible", config->adjustDists.drible);
    get_parameter("striker.adjust_dists.tolerance", config->adjustDists.tolerance);
...
void Brain::handleCooperation() {
    ...
    log_(format("entrei no mudando de posicao"));
    ...
    } else if(msecsSince(data->leadSwitchTimer) > config->leadSwitchThreshold) {

        if (data->tmImLead == false)
        {
            data->leadSwitchTimer = get_clock()->now();
        }
        data->tmImLead = true;
...
void Brain::updateBallMemory()
{
    ...
    data->ballBuffer.insert(data->ballBuffer.begin(), data->ball);
...
void Brain::updateCostToKick() {
    ...
    // se ele não sabe onde esta a bola
    if (!tree->getEntry<bool>("ball_location_known")) {
        cost += 5.0;
        log_(format("ball lost cost: %.1f", 5.0));
    }

    // se tiver obstaculos dentro do raio 
    ...
    // se nao tiver localizado
    if (!tree->getEntry<bool>("odom_calibrated")) {
        cost += 100;
        log_(format("localization cost: %.1f", 100.0));  
    }
    
    // itera sobre os jogadores e avalia a posição deles em relação a bola
    ...
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; i++) {
        ...
        // avalia a distancia lateral entre o colega e a bola e a minha distancia com a bola 
    ...
    // atualiza o custo dependendo do quanto devo ajustar meu angulo pra chutar
    ...
    // aumenta o custo de acordo com o theta da bola em relacao a mim
    cost += fabs(data->ball.yawToRobot) / 1.0; 
    log_(format("ball yaw cost: %.1f", fabs(data->ball.yawToRobot) / 1.0));

    // aumenta o custo de acordo com os segundos contados apos detecção da bola
    ...
    // aumenta o custo dependendo da distancia com a bola 
...
{
    using namespace booster_internal::robot::b1;
    auto log_ = [=](string msg) {
        log->setTimeNow();
    ...
    static double kickSpeed = 1.0;

    // if (data->currentRobotModeIndex == 8)
    // {
    //     if (joy.a)
    //     {
    //         client->highKick();
    ...
    //     else if (joy.b)
    //     {
    //         client->enableVisualKickMode();
    //     }
    //     else if (joy.x)
    //     {
    //         client->rlKickBall(kickSpeed, 1.0);
    //     }
    //     else if (joy.y)
    //     {
    //         client->fancyKick(kickSpeed, 0.1);
    //     }
    //     else if (joy.lb)
    //     {
    //         SquatDirection dir = kSquatDown;
    //         client->squatAction(dir);
    //     }
    //     else if (joy.rb)
    //     {
    //         SquatDirection dir = kSquatUp;
    //         client->squatAction(dir);
    //     }
    //     else if (joy.lt)
    //     {
    //         SquatDirection dir = kSquatDown;
    //         SquatSide side = kLeft;
    //         client->goalieSquatDown(dir, side);
    //     }
    //     else if (joy.rt)
    //     {
    //         SquatDirection dir = kSquatDown;
    //         SquatSide side = kRight;
    //         client->goalieSquatDown(dir, side);
    //     }
    //     else if (joy.hat_d)
    //     {
    ...
    if (joy.lt && !joy.rt) { // LT 组合键
        ...
        // 用于控制切换不同的状态 maquina de estados
        if (joy.x)
        {
...
void Brain::gameControlCallback(const game_controller_interface::msg::GameControlData &msg)
{
    ...
    // 处理比赛的一级状态       estado primario de jogo
    ...
    // FOR FUN 处理进球后的庆祝挥手的逻辑       comemora o gol
...
Point2D Brain::findDribleDir(vector<Point2D> &detectedOpponents)
{
    auto opponents = data->getRobots();
    Point2D oppPos;
    Point2D bPos = Point2D{data->ball.posToField.x,
                            data->ball.posToField.y};
    vector<Point2D> aux, opponentAngles;
    opponentAngles.clear();

    int rangeBegin = 0, rangeSize = 0,
        maxRangeBegin = 0, maxRangeSize = 0;
    ...
    for (auto &opp : opponents)
    {
        ...
        oppPos.pointInsideCircle(config->dribleRadius, bPos,
                                config->dribleCircleDegrees,
                                data->robotPoseToField.theta, aux);

        if (opponentAngles.empty())
        {
            opponentAngles = aux;
        }
        else
        {
            ...
            {
                if (aux.at(n).norm() > 0.001)
                {
                    opponentAngles.at(n) = aux.at(n);
                }
            }
        }
    }
    ...
    if (opponentAngles.empty())
    {
        return {config->fieldDimensions.length / 2.0, 0};
    }

    for (int n = 0; n < opponentAngles.size(); n++)
    {
        if (opponentAngles.at(n).norm() > 0.001)
        {
            if (rangeSize > maxRangeSize)
            {
                maxRangeSize = rangeSize;
                maxRangeBegin = rangeBegin;
            }
            rangeBegin = n+1;
            rangeSize = 0;
        }
    ...
    int driblePosIdx = min(static_cast<size_t>(maxRangeBegin + maxRangeSize / 2),
                           opponentAngles.size()-1);

    Point2D driblePos = opponentAngles.at(driblePosIdx);
    detectedOpponents = opponentAngles;
    return driblePos;
}
